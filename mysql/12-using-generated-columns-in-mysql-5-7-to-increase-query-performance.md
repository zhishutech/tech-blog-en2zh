...
在这篇博客中，我们将看看如何使用MySQL 5.7的生成列（或虚拟列）来提高查询性能。
...
### 说明
大约两年前，我发表了一个在MySQL5.7版本上关于生成列（虚拟列）的博客文章。从那时开始，它成为MySQL5.7发行版当中，我最喜欢的一个功能点。原因很简单：在虚拟列的帮助下，我们可以创建fine-grained indexes，可以显著提高查询性能。我要告诉你一些技巧，可以潜在地解决那些使用了GROUP BY 和 ORDER BY而慢的报表查询。
...
这个查询运行了一个多小时，并且使用和撑满了整个 tmp目录（用于文件排序）。
...
总行数：21998514。查询需要扫描的总行数的36%（7948800/21998514）。
...
MySQL可以避免排序，速度更快。它将最终还是要扫描所有表的索引的顺序。响应时间明显更好：~ 38秒而不是>一小时。
...
我们添加了一个“动词”和“URL”，所以之前我不得不删除表定义的COLLATE utf8mb4_unicode_ci。现在执行计划表明，我们使用了覆盖索引：
...
1 row in set, 1 warning (0.00 sec)
```
响应时间下降到~12秒！但是，索引的大小明显地比仅verb_url_hash的索引（每个记录16字节）要大得多。
